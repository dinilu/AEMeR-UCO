...
::: {.callout-note}
He modificado los datos originales de Michelle en varios sentidos. Por un lado, he rellenado varios datos de las variables ambientales que no se habían podido medir. Por otro lado, había varios puntos de muestreo que no presentaban ningún individuo de las especies estudiadas. En estos casos, he seleccionado una especie al azar y le he añadido un individuo. Esto no es correcto, pero los análisis de ordenación no aceptan puntos sin observación alguna, ni datos ausentes. Lo he hecho intencionadamente para que los datos de Michelle no estén en bruto y podamos realizar los ejercicios sin preocuparnos de los datos NA o sin observaciones.
:::
...
**2. Vamos a preparar un poco los datos. Los datos de comunidades son datos de abundancia, por lo que tienen una distribución muy sesgada hacia la izquierda (valores próximos a cero). En estos casos, es frecuente aplicar una transformación para reducir dicho sesgo. Utiliza la función `log1p()` sobre la matriz `com`. El resultado puedes llamarlo `comm`. Además, la tabla (*data frame*) `env` tiene varias columnas y sólo 3 de ellas son variables ambientales. Las otras dos son información que caracteriza el punto de muestreo (información del río, més de muestreo y ubicación con respecto a la desembocadura). Genera dos *data frame* distintos; uno llamado `sites` que tenga la información de caracterización del punto de muestreo (`rio`, `sitio` y `mes`)y otro llamado `envm` con los datos de las variables ambientales medidas (`materia_org`, `temperatura`, `salinidad` y `ph`).**
...
**3. Como hemos comentado en clase, una primera aproximación puede ser realizar un análisis DCA para saber si en nuestros datos dominan las respuestas lineares o unimodales. Ejecuta un DCA sobre los datos de las comunidades transformados (`comm`).**
...
**5. Ahora, realiza un análisis *constrained* del conjunto de datos `comm`, incorporando el objeto `envm` al análisis PCA. Consulta la inercia total, y la proporción explicada por la parte *constrained* y *unconstrained*. Finalmente, dibuja el gráfico de ordenación resultante. ¿Qué variables explican el principal eje de variación en cuanto a composición de los puntos de muestreo?**

::: {.callout-tip collapse="true"}
## Pista 

Para calibrar el modelo RDA, basta con incorporar un segundo argumento a la función `rda()` con el *data frame* de variables ambientales.
:::
...
Aqui deberas usar la función `vegdist` sobre el objeto de comunidades original.
:::
...
**7. Realiza un análisis *clúster* en base a la matriz de diferencias que se generó en el ejercicio anterior. Para ello, modifica el argumento `method` para que muestre los valores (e.gr. .**

::: {.callout-tip collapse="true"}
## Pista 

Para realizar el análisis necesitas la función `hclust()`.
:::
...
**8. Grafica el análisis *clúster* y usa el argumento `hang = -1` para que dibuje el gráfico de manera más amigable con el usuario. Posteriormente, añade una serie de rectángulos en el gráfico agrupando con los puntos de muestreo que son similares entre si.**

::: {.callout-tip collapse="true"}
## Pista 

Aquí puedes consultar la función `rect.hclust`. En esta función podrás elegir usar un valor de umbral (`h = xxx`) o un número preestablecido de grupos (`n = xxx`). 
:::
...
**9. Usando el mismo sistema que antes, genera un vector con el grupo resultante de cada sitio.**

::: {.callout-tip collapse="true"}
## Pista 

Aquí deberás usar la función `cutree()` especifican el número de grupos igual que antes.
:::
...
**9. Por último, genera de nuevo el gráfico del resultado del análisis *constrained* RDA. Dibuja el biplot del análisis RDA e incorpora la información de los grupos identificados con el análisis de clasificación.**
