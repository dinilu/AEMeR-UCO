...
El paquete `gdm` tiene una forma particular de funcionar y especificar los datos. Así que necesitamos prepararlos un poco antes de empezar
...
head(gdmRast.sp_trans)
```

```{r}
#| echo: true

terra::plot(gdmRast.sp_trans, col=colorRamps::rgb.tables(1000))
```
...
pca.rast <- prcomp(terra::rast(gdmRast.sp_trans))

# note the use of the 'index' argument
pca.rast <- terra::predict(gdmRast.sp_trans, pca.rast, index=1:3)
pca.rast <- terra::rast(pca.rast)
# scale rasters
pca.rast[[1]] <- terra::app(pca.rast[[1]], fun = scales::rescale, to = c(0, 255))
pca.rast[[2]] <- terra::app(pca.rast[[2]], fun = scales::rescale, to = c(0, 255))
pca.rast[[3]] <- terra::app(pca.rast[[3]], fun = scales::rescale, to = c(0, 255))
